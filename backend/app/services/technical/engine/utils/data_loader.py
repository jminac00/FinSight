...
# ---------------------------------------------------------------------------
# Datos sectoriales — FMP
# ---------------------------------------------------------------------------

# Mapeo sector yfinance → nombre de campo en la respuesta FMP
_SECTOR_FIELD_MAP: dict[str, str] = {
    "Technology": "technologyChangesPercentage",
    "Information Technology": "technologyChangesPercentage",
    "Healthcare": "healthcareChangesPercentage",
    "Health Care": "healthcareChangesPercentage",
    "Financials": "financialsChangesPercentage",
    "Financial Services": "financialsChangesPercentage",
    "Communication Services": "communicationServicesChangesPercentage",
    "Consumer Discretionary": "consumerCyclicalChangesPercentage",
    "Consumer Staples": "consumerDefensiveChangesPercentage",
    "Industrials": "industrialsChangesPercentage",
    "Energy": "energyChangesPercentage",
    "Real Estate": "realEstateChangesPercentage",
    "Materials": "basicMaterialsChangesPercentage",
    "Utilities": "utilitiesChangesPercentage",
}


def get_sector_performance(
    sector: str,
    period_months: int = 13,
    api_key: str | None = None,
) -> pd.Series | None:
    """
    Obtiene los retornos diarios históricos del sector desde FMP.

    Intenta primero el endpoint 'stable'; si falla, prueba el v3.
    Devuelve pd.Series con retornos diarios (en decimal, no porcentaje)
    indexada por fecha, o None si la API no está disponible o falla.
    """
    if not api_key:
        return None

    field = _SECTOR_FIELD_MAP.get(sector)
    if field is None:
        logger.debug("Sector '%s' no tiene mapeo en FMP.", sector)
        return None

    limit = period_months * 23  # días de trading aproximados + margen
    ...
    for url in endpoints:
        try:
            ...
            series = pd.to_numeric(df[field], errors="coerce").dropna() / 100.0
            logger.debug("FMP sector '%s' cargado: %d sesiones.", sector, len(series))
            return series

        except Exception as exc:
            logger.debug("FMP endpoint '%s' fallo: %s", url[:60], exc)
            continue

    logger.warning("FMP API no disponible para sector '%s'.", sector)
    return None
